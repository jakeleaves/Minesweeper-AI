...
class Window(Frame):
    ...
    def init_window(self, runMethod):
        ...
        # self.AI = None #I am making modifications here 
        if self.AIType == 1:
    ...
    def AIAlgo2(self):
        ''' OUR CHANGES TO THIS METHOD '''
        startTime = time.time()
        gridInput = ""
        for row in self.ans:
            for val in row:
                gridInput = gridInput + str(val)

        boardState = self.getBoardState()
        userCommand1, userCommand2 = self.AI2.performAI(boardState, self.safeSquare, gridInput)
        endTime = time.time()

        self.time += (endTime - startTime)

        for x in userCommand1:
            want = x[1] 
            self.open_button(want[0], want[1])

        print("my bombs found")
        print(userCommand2)
        self.parseAIAlgo(userCommand2)
